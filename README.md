...
`plugin__openshift-camel-console-plugin` namespace. You can use the `useTranslation` hook
with this namespace as follows:

```tsx
conster Header: React.FC = () => {
  const { t } = useTranslation('plugin__openshift-camel-console-plugin');
  return <h1>{t('Hello, World!')}</h1>;
};
```

For labels in `console-extensions.json`, you can use the format
`%plugin__openshift-camel-console-plugin~My Label%`. Console will replace the value with
the message for the current language from the `plugin__openshift-camel-console-plugin`
...
```json
  {
    ...
    "properties": {
      ...
      "perspective": "admin",
      "name": "%plugin__openshift-camel-console-plugin~Plugin Template%"
    }
  }
```
