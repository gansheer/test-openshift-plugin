...
const PLUGIN_TEMPLATE_NAME = 'test-openshift-plugin';
...
const installHelmChart = (path: string) => {
  cy.exec(
    `cd ../../test-openshift-plugin && ${path} upgrade -i ${PLUGIN_TEMPLATE_NAME} charts/test-openshift-plugin -n ${PLUGIN_TEMPLATE_NAME} --create-namespace --set plugin.image=${PLUGIN_TEMPLATE_PULL_SPEC}`,
    {
...
const deleteHelmChart = (path: string) => {
  cy.exec(
    `cd ../../test-openshift-plugin && ${path} uninstall ${PLUGIN_TEMPLATE_NAME} -n ${PLUGIN_TEMPLATE_NAME} && oc delete namespaces ${PLUGIN_TEMPLATE_NAME}`,
    {
      failOnNonZeroExit: false,
    },
  ).then((result) => {
...
describe('Console plugin template test', () => {
  before(() => {
    ...
    if (!isLocalDevEnvironment) {
      ...
      cy.exec('cd ../../test-openshift-plugin && ./install_helm.sh', {
        failOnNonZeroExit: false,
      }).then((result) => {
  ...
  it('Verify the example page title', () => {
    ...
    cy.get('[data-test="nav"]').contains('Camel').click();
