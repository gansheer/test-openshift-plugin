...
const PLUGIN_TEMPLATE_NAME = 'openshift-camel-console-plugin';
...
const installHelmChart = (path: string) => {
  cy.exec(
    `cd ../../openshift-camel-console-plugin && ${path} upgrade -i ${PLUGIN_TEMPLATE_NAME} charts/openshift-camel-console-plugin -n ${PLUGIN_TEMPLATE_NAME} --create-namespace --set plugin.image=${PLUGIN_TEMPLATE_PULL_SPEC}`,
    {
...
const deleteHelmChart = (path: string) => {
  cy.exec(
    `cd ../../openshift-camel-console-plugin && ${path} uninstall ${PLUGIN_TEMPLATE_NAME} -n ${PLUGIN_TEMPLATE_NAME} && oc delete namespaces ${PLUGIN_TEMPLATE_NAME}`,
    {
      failOnNonZeroExit: false,
    },
  ).then((result) => {
...
describe('Console plugin template test', () => {
  before(() => {
    ...
    if (!isLocalDevEnvironment) {
      ...
      cy.exec('cd ../../openshift-camel-console-plugin && ./install_helm.sh', {
        failOnNonZeroExit: false,
      }).then((result) => {
