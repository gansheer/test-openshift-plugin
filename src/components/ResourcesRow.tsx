...
import Status from '@openshift-console/dynamic-plugin-sdk/lib/app/components/status/Status';
  ...
  const ResourcesRow: React.FC<RowProps<K8sResourceKind>> = ({
    obj: resource,
    activeColumnIDs,
  }) => {
    ...
    const applicationUrl = "/camel/application/"+resource.metadata.namespace+"/"+resource.kind+"/"+resource.metadata.name;
      ...
      <>
        ...
        <TableData id="kind" activeColumnIDs={activeColumnIDs}>
          <span className="co-break-word co-line-clamp">
            {getKind(resource) || (
              <span className="text-muted">{t('No kind')}</span>
            )}
          </span>
        </TableData>
        <TableData id="namespace" activeColumnIDs={activeColumnIDs}>
          <span className="co-break-word co-line-clamp">
            {getNamespace(resource) || (
              <span className="text-muted">{t('No namespace')}</span>
            )}
          </span>
        </TableData>
